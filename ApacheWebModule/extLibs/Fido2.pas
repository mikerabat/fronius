...
type
  TFidoDevice = class;
  TFidoDevList = {$IFDEF FPC} specialize {$ENDIF} TObjectList<TFidoDevice>;
  ...
  public
    ...
    // constructors
    constructor CreateFromPath( aUsbPath : String );
...
type
  ...
  TFidoBiometricTemplateArr = {$IFDEF FPC} specialize {$ENDIF} TObjectList<TFidoBiometricTemplate>;
...
type
  ...
  public
    function Verify( aClientDataHash : TFidoSHA256Hash ) : boolean;
    ...
    constructor Create( typ: TFidoCredentialType; aFmt: TFidoCredentialFmt;
...
type
  ...
  public
    ...
    constructor Create( rp : Pfido_credman_rp_t; aIdx : size_t );
    destructor Destroy; override;
  end;
...
type
  TFido2RelayPartyList = {$IFDEF FPC} specialize {$ENDIF} TObjectList<TFido2RelayParty>;
  TFido2ResidentKeyList = {$IFDEF FPC} specialize {$ENDIF} TObjectList<TFido2ResidentKey>;
...
begin
     loc_fidoLog := OnLog;
     fido_set_log_handler({$IFDEF FPC}@{$ENDIF}FidoLog);
end;
...
constructor TFidoDevice.CreateFromPath(aUsbPath: String);
begin
     fUSBPath := UTF8String(aUsbPath);
     OpenDevice;
...
begin
     ...
     fDevFlags := [];
     if devFlags and FIDO_CAP_WINK <> 0 then
        include(fDevFlags, dfWink);
     if devFlags and FIDO_CAP_CBOR <> 0 then
        include(fDevFlags, dfCBOR);
     if (devFlags and FIDO_CAP_NMSG) = 0 then
...
function TFidoCredVerify.Verify(aClientDataHash: TFidoSHA256Hash): boolean;
var r : integer;
begin
     if Length(fAuthData) = 0 then
     ...
     fClientDataHash := aClientDataHash;
     PrepareCredentials;
...
constructor TFidoCredVerify.Create(typ: TFidoCredentialType; aFmt: TFidoCredentialFmt;
  aRelyingPartyID, aRelyingPartyName : string;
...
constructor TFido2RelayParty.Create( rp : Pfido_credman_rp_t; aIdx : size_t );
var pS : PAnsiChar;
    len : integer;
begin
     inherited Create;
...
begin
     ...
     cnt := fido_credman_rk_count( rk );
     SetLength(fCredList, cnt);
     for idx := 0 to cnt - 1 do
     begin
          aCred := fido_credman_rk( rk, idx );
          if aCred <> nil then
             fCredList[idx] := TFido2Credential.Create( aCred );
     end;
end;
...
begin
     ...
     pFmt := fido_cred_fmt( cred );
     fFmt := fmNone;

     if pFmt <> nil then
     begin
          aFmt := UTF8ToString(pFmt);
